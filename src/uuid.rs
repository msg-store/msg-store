...
impl Uuid {
  ...
  pub fn from_string(string: &str) -> Uuid {
    ...
    Uuid {
      priority: file_name_str_vec[0].parse::<u64>().expect("Could not parse priority"),
      time_stamp: file_name_str_vec[1].parse::<u128>().expect("Could not parse time_stamp"),
      sequence: file_name_str_vec[2].parse::<u64>().expect("Could not parse sequence")
    }
  }
  pub fn from_vec(array: &Vec<&str>) -> Uuid {
    Uuid {
      priority: array[0].parse::<u64>().expect("Could not parse priority"),
      time_stamp: array[1].parse::<u128>().expect("Could not parse time_stamp"),
...
impl AsRef<[u8]> for Uuid {
    fn as_ref(&self) -> &[u8] {
        ...
        use std::mem::size_of;
        unsafe { 
            from_raw_parts(transmute(self), size_of::<Uuid>())
        }
    }
}
...
impl PartialEq for Uuid {
  fn eq(&self, other: &Self) -> bool {
    self.priority == other.priority && self.time_stamp == other.time_stamp && self.sequence == other.sequence
  }
}
...
impl IdManager {
  pub fn new() -> IdManager {
    IdManager {
      sequence: 0,
      time_stamp: time::SystemTime::now().duration_since(time::UNIX_EPOCH).unwrap().as_millis()
    }
  }
  pub fn new_id(&mut self, priority: &u64) -> Uuid {
    let time_stamp = time::SystemTime::now().duration_since(time::UNIX_EPOCH).unwrap().as_millis();
